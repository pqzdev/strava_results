  ...
  try {
    ...
    if (!response.ok) {
      if (response.status === 450) {
        throw new Error(
          `Parkrun blocked the request (HTTP 450). Parkrun actively prevents automated scraping. ` +
          `Please contact api@parkrun.com for official API access, or use manual CSV import instead.`
        );
      }
