...
export async function triggerAthleteSync(
  ...
  try {
    ...
    const sessionId = `sync_${athleteId}_${Date.now()}`;
...
/**
 * GET /api/admin/sync-logs?session_id=X - Get sync logs for a session
 */
export async function getSyncLogsForSession(request: Request, env: Env): Promise<Response> {
  try {
    const url = new URL(request.url);
    ...
    // Try to get logs, but handle case where table doesn't exist yet
    let logs = [];
    try {
      logs = await getSyncLogs(env, sessionId, 500);
    } catch (error) {
      // If sync_logs table doesn't exist yet, return empty array
      console.log('Sync logs table may not exist yet:', error);
      logs = [];
    }
